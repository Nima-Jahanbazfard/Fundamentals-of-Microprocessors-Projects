...
while (1)
      ...
      if(PINC.0==0){
      PORTB=0x00;
      while(PINC.0==0){}//PINC.0 THE KEY FOR CLOSING DOORS
      lcd_clear();
      lcd_puts("Doors are closed");
      delay_ms(1000);
      lcd_clear();
      lcd_puts("Close seatbelt");
      strcpy(lcdText,"Close seatbelt");
      delay_ms(1000);
      } 
      if(PINC.1==0){
      while(PINC.1==0){}
      lcd_clear();
      lcd_puts("Doors still open");}
      if(strcmp(lcdText,"Close seatbelt")==0 && PINC.1==0)//PINC.1 THE KEY FOR CLOSING SEATBELT
      {
      while(PINC.1==0){}
      lcd_clear();
      lcd_puts("Ready");
